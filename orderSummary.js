const tellimus = {
  ...
  function orderSummary(array) {
    for (let i = 0; array.rows[i]; i++) {
      console.log(array.rows[i].name);
      console.log(array.rows[i].amount);
      console.log(array.rows[i].price);
      console.log(array.rows[i].price * array.rows[i].amount);
    }
    console.log(
      array.rows.reduce(
        (sum, { price, amount, vat }) => sum + price * amount * (1 + vat),
        0
      )
    );
  }
